...
* **stack**: A string that, if provided, will set the specific language environment. If blank the language version will be set to default language version defined in runtime. [See More Information on Stack settings](https://dev.iron.io/worker/reference/environment/#default_language_versions).
...
* **Content-Type**: Should be set to "multipart/form-data ; boundary={Insert Value Here}" with boundary set to [an appropriate value](https://en.wikipedia.org/wiki/MIME#Multipart_messages).
